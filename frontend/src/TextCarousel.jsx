...
function TextCarousel({ showInput, toggleShowInput }) {
  ...
  const handleSubmit = async () => {
    if (text.trim().length > 0) {
      try {
        const response = await fetch('https://word-ether2-642016a371b5.herokuapp.com/poems', {
          method: 'POST',
          headers: {
  ...
  const handleLike = async () => {
    if (currentPoem && currentPoem._id) {
      try {
        let url = `https://word-ether2-642016a371b5.herokuapp.com/poems/${currentPoem._id}/`;
